...
module.exports = {
    ...
    validate(channel, name, cmd, args, callback) {
		sql.getPlayerByUsername(channel, name, player => {
			var targetName;
			if(cmd == 'use') {
			...
			sql.getPlayer(channel, targetName, target => {
				sql.getNemesis(channel, nemesis => {
					sql.getGarden(channel, garden => {
						var errors = [];
						...
						switch(cmd) {
							...
							case 'fight':
								...
								if(player) {
									var defeated = player.status.find(s => s.type == 0);
									if(defeated) {
										var timeString = tools.getTimeString(defeated.endTime - now);
									...
									if(target) {
										...
										var targetDefeated = target.status.find(s => s.type == 0);
										if(targetDefeated) {
											var timeString = tools.getTimeString(targetDefeated.endTime - now);
								...
								if(player) {
									...
									if(nemesis) {
										if(nemesis.attackTime > now) {
											var timeString = tools.getTimeString(nemesis.attackTime - now);
							...
							case 'pick':
								...
								if(targetName) {
									var plantType = module.exports.getPlantType(targetName);
									var plantCount = garden.plants.filter(p => p && p.type == plantType && p.growTime < now).length;
									if(plantType == -1) {
									...
									var plantCount = garden.plants.filter(p => p && p.growTime < now).length;
									if(plantCount == 0) {
							...
							case 'plant':
								...
								if(player) {
									...
									var plantCount = garden.plants.filter(p => p).length;
									if(plantCount == 3) {
							...
							case 'research':
								...
								if(player) {
									module.exports.validateNotNemesis(errors, player);
									module.exports.validateGardenTime(errors, player);
									var knownPlants = garden.plantTypes.filter(t => t.known).length;
							...
							case 'nemesis':
								...
								if(player) {
									if(nemesis.cooldown > now) {
										var timeString = tools.getTimeString(nemesis.cooldown - now);
							...
							case 'wish':
								...
								if(player) {
									...
									var orbs = player.items.find(i => i.type == 0);
	...
	validateActionTime(errors, player) {
		...
		if(player.actionTime > now) {
			var timeString = tools.getTimeString(player.actionTime - now);
	...
	validateGardenTime(errors, player) {
		...
		if(player.gardenTime > now) {
			var timeString = tools.getTimeString(player.gardenTime - now);
